...
% catch case of not enough usable data                 GK, 22.05.2015  0.8-->0.9
% replace julian/gregorian with datenum/datevec        GK, 10.02.2017  0.9-->1
...
if ps.dragfac>0
  ...
  for j=1:length(shipdragvel)
    ...
    nsmooth = sum(di.time_datenum>(di.time_datenum(j)-ps.drag_lag_tim/24/60) & di.time_datenum<di.time_datenum(j));
...
%
% catch too little data case
%
if isempty(Aocean)
  disp(' ')
  disp(' ')
  warning('>>  Not enough data for processing. Terminating...')
  der = [];
  de = [];
  return
end
...
% catch too little data case
%
if size(de.ocean_constraints,2)==1
  disp(' ')
  disp(' ')
  warning('>>  Not enough data for processing. Terminating...')
  der = [];
...
fac = sqrt(sum(range));
disp(['    Normalized barotropic constraint weight: ',num2str(w)])
...
%function [Aocean,Ac,d]=lainocean(Aocean,Ac,d,w)
%
% set barotropic constraint to be no mean ocean velocity
% w strength of constraint
%
%
[li,ljo]=size(Aocean);
...
for i=1:length(izz);
  ...
  % set U_CTD = - ctdvel as constraint
