...
% params.use_3beam_w_for_lag                    GK, 14.05.2013  0.17-->0.18
% fill gaps with NaN instead of 0's. This might cause problems elsewhere,
% but does not give wrong values                GK, 11.07.2013  0.18-->0.19
% fixed bug in b2earth                          GK, 26.10.2013  0.19-->0.20
% replaced our b2earth with that of Andreas Thurnherr.
% This allows for 3beam solutions, and has bin remapping permanently
% removed                                       GK, 21.11.2013  0.20-->0.21
% fixed bug in text output                      GK, 27.11.2013  0.21-->0.22
% p.timoff_downlooker                           GK, 02.12.2013  0.22-->0.23
% blank length output on the screen             GK, 20.06.2014  0.23-->0.24
% more screen info                              GK, 17.06.2015  0.24-->0.25
% set bandwidth to 0 for NB                     GK, 10.07.2015  0.25-->0.26
% show check plot also for timoff_downlooker    GK, 14.07.2015  0.26-->0.27
% handle case of full data set in downlooker but only a stopped uplooker
% after say half the profile                    GK, 16.06.2016  0.27-->0.28
% replace julian/gregorian with datenum/datevec GK, 10.02.2017  0.28-->1
...
f.dist = 5;   % distance to the center of the first depth cell
...
if isbb(fid_dn)
  ...
  if fd(f.blnk)==0 && isempty(params.edit_mask_dn_bins)
    ...
    disp(['    blank length downlooker: ',num2str(fd(f.blnk)),' m'])
  end
...
if ~isempty(params.edit_hardremove_mask_dn_bins);
  disp('    Removing bins from dataset')
...
if dd.Coordinates==0
  ...
  dd.beam2earth_allow_3beam_solutions = params.beam2earth_allow_3beam_solutions;
  dd.beam2earth_bad_beam = params.beam2earth_bad_down_beam;
...
if data.btrk_used == 1
  ...
  if db.Coordinates==0
    ...
    db.use_binremap = 0;
    db.beam2earth_allow_3beam_solutions = params.beam2earth_allow_3beam_solutions;
    db.beam2earth_bad_beam = params.beam2earth_bad_down_beam;
...
%
% change time of downlooker
%
if params.timoff_downlooker~=0
  vd(:,1,v.tim) = vd(:,1,v.tim)+params.timoff_downlooker;
end
...
%    time_matching = 1;
  ...
  if fu(f.blnk)==0 && isempty(params.edit_mask_up_bins)
    ...
    disp(['    blank length uplooker: ',num2str(fu(f.blnk)),' m'])
  end

  % special treatment of one particular instrument
  % we simply remove the first N pings
  if params.remove_first_pings_of_uplooker~=0

    % look for a large time jump and remove everything before the jump
    if params.remove_first_pings_of_uplooker==-1
      ddt = diff(vu(:,v.tim))*86400;
      ind = find(ddt>100);
      if length(ind)==1
        disp(['>   Found time jump of ',num2str(ddt(ind)),' seconds.'])
        disp(['>   Removing ALL ',int2str(ind),' pings before jump.'])
        params.remove_first_pings_of_uplooker = ind+1;
      else
        params.remove_first_pings_of_uplooker = 1;
      end
    end
    disp(['>   Removing first ',int2str(params.remove_first_pings_of_uplooker),...
      ' pings of uplooker'])
    vu = vu(params.remove_first_pings_of_uplooker:end,:,:);
    velu = velu(params.remove_first_pings_of_uplooker:end,:,:);
    cmu = cmu(params.remove_first_pings_of_uplooker:end,:,:);
    eau = eau(params.remove_first_pings_of_uplooker:end,:,:);
    pgu = pgu(params.remove_first_pings_of_uplooker:end,:,:);
    btu = btu(params.remove_first_pings_of_uplooker:end,:,:);
  end
  ...
  if du.Coordinates==0
    ...
    du.beam2earth_allow_3beam_solutions = params.beam2earth_allow_3beam_solutions;
    du.beam2earth_bad_beam = params.beam2earth_bad_up_beam;
...
if values.up==1
  ...
  if params.timoff_uplooker~=0 | params.timoff_downlooker~=0
    sfigure(3);
    clf
    ...
    disp(' ')
    disp('PAUSED')
    disp('check whether params.timoff_up/downlooker brings vertical velocities to alignment')
    disp(' ')
    pause
    close(3)
  end
  ...
  if vu(1,1,v.tim)-vd(1,1,v.tim)>10/86400
    ...
    vu(1:extra_ind,1,v.pit) = nan;
    vu(1:extra_ind,1,v.rol) = nan;
    vu(1:extra_ind,1,v.hdg) = nan;
  ...
  elseif vd(1,1,v.tim)-vu(1,1,v.tim)>20/86400
    ...
    vd(1:extra_ind,1,v.pit) = nan;
    vd(1:extra_ind,1,v.rol) = nan;
    vd(1:extra_ind,1,v.hdg) = nan;
...
%    error('this is not yet coded. Contact Gerd Krahmann for help.')
  ...
  if params.use_3beam_w_for_lag~=1
    dummy = squeeze(velu(iu,:,4));
    bad = find(isnan(dummy));
    if length(find(~isnan(dummy)))<0.2*length(find(~isnan(wu(:))))
      disp('>   found very large number of no data or 3 beam solutions. Consider setting ''params.use_3beam_w_for_lag''')
    end
    if ~isempty(bad)
      wu(bad) = nan;
    end
  ...
  if params.use_3beam_w_for_lag~=1
    ...
    bad = find(isnan(dummy));
    if length(find(~isnan(dummy)))<0.2*length(find(~isnan(wd(:))))
      disp('>   found very large number of no data or 3 beam solutions. Consider setting ''params.use_3beam_w_for_lag''')
    end
    if ~isempty(bad)
      wd(bad) = nan;
  ...
  sfigure(3);
  clf
  plot(vd(:,1,v.tim),vd(:,1,v.hdg))
  hold on
  plot(vu(:,1,v.tim),vu(:,1,v.hdg),'r')
  ...
  if ~isnan(params.set_lag_up2down)
    lag = round(params.set_lag_up2down);
    l1 = length(wb2u);
    l2 = length(wb2d);
    i1 = 1:l1;
    i2 = i1+lag;
...
% distance to surface
  ...
  % Check whether we have a data extrapolation problem.
  % This happens when the slave stopped recording for whatever reason.
  % The time matching will replicate the last data set of the slave for
  % all subsequent time steps. We have to set the relevant values for
  % these to NaN.
  most_frequent_up_time = mode(data.tim(2,:));
  ind = find(data.tim(2,:)==most_frequent_up_time);
  if length(ind)>5   % now we are sure to have the problem
    data.hdg(2,ind) = nan;
    hdg_offset = nmedian(diff(data.hdg));
    data.hdg(2,ind) = data.hdg(1,ind) + hdg_offset;
    bad = find(data.hdg(2,:)>=360);
    if ~isempty(bad)
      data.hdg(2,bad) = data.hdg(2,bad) - 360;
    end
    bad = find(data.hdg(2,:)<0);
    if ~isempty(bad)
      data.hdg(2,bad) = data.hdg(2,bad) + 360;
    end
    data.u([1:size(velu,2)],ind) = nan;
    data.v([1:size(velu,2)],ind) = nan;
    data.w([1:size(velu,2)],ind) = nan;
    data.e([1:size(velu,2)],ind) = nan;
  end
...
for n=1:size(data.tim,1)
  time_datevec = datevec(data.tim(n,:));
  if time_datevec(1)==1900
    if isfield(params,'correct_year')
      time_datevec(:,1) = params.correct_year;
    else
      disp('>   Found Narrowband with wrong year.')
      error('Please set params.correct_year in cast_params.m')
    end
    disp('>   Narrowband : found year 1900, correcting with preset year')
  end
  data.tim(n,:) = datenum(time_datevec)';
end
...
%
% catch odd timings in downlooker when resampled
%
if any(diff(data.tim(1,:))==0)
  if values.resampling==2
    data.tim(1,:) = data.tim(2,:);
    disp('>   Downlooker was resampled and double times occurred.')
    disp('>   Replacing down time with up time.')
  end
end
...
C30=cos(p.Beam_angle*d2r);
S30=sin(p.Beam_angle*d2r);

if p.Up == 1, % for upward looking
  ZSG = [+1, -1, +1, -1];
...
%      end
    ...
    if all(J > 0) & all(J <= NBINS),
      ...
      if ~isnan(p.beam2earth_bad_beam)
        beam(:,ignore_beam) = nan;
      end

      this_3beam = 0;
      if (p.beam2earth_allow_3beam_solutions) && ...
...
% the remainder is not in use any more as of version 10.19

%-------------------------------------------------------------------------------



function [vele]=b2earth_old(velb,v,p)
% 
% convert beam ADCP data to earth velocities
%
% input velb:  beam coordinates
...
%  end
  ...
  %[NBINS] = size(beam);    changed GK, 26.10.2013
  [NBINS] = size(beam,1);
  earth = zeros(size(beam));
  J = zeros(1,4);
