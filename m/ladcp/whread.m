...
% check for backward time jump                    GK, 04.05.2015  0.3-->0.4
% replace julian/gregorian with datenum/datevec   GK, 10.02.2017  0.4-->1
...
%
% introduced the following after finding a slave file where the time jumped
% backwards   GK, May 2015
%
bad = find( diff( varargout{2}(:,1,1) )<0 );
if ~isempty( bad )
  good = [1:bad(1)];
  disp(['>   Found backward time jump after ensemble ',int2str(bad(1))])
  disp('>     removing all ensembles after the jump and continuing with the rest')
  varargout{2} = varargout{2}(good,:,:);
  varargout{3} = varargout{3}(good,:,:);
  varargout{4} = varargout{4}(good,:,:);
  varargout{5} = varargout{5}(good,:,:);
