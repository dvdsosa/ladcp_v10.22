...
if isfield(data,'wctd')==1
  ...
  data.wm = -gradient(data.z)./gradient(data.time_datenum*86400);
end
...
if (sum(ii)~=length(ii)) & params.cut>0 
  ...
  params.time_start = datevec(data.time_datenum(1));
  params.time_end = datevec(data.time_datenum(end));
...
if ~isempty(data.ctdtime_data)
  ...
  ax = axis;
  plot((data.time_datenum(1)-data.ctdtime_time(1))*24*[1,1], ax(3:4),'r--')
  plot((data.time_datenum(end)-data.ctdtime_time(1))*24*[1,1], ax(3:4),'r--')
  axis tight
end
