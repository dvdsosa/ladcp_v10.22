...
%
% version 0.6	last change 11.07.2013
...
% replaced mean with nmean in compoff     GK, 11.07.2013  0.5-->0.6
...
if values.up==1
  ...
  if all(isnan(hdg_down))
    hdg_down = hdg_up;
    d.hdg(1,:) = d.hdg(2,:);
    d.pit(1,:) = d.pit(2,:);
    d.rol(1,:) = d.rol(2,:);
    disp('>   Found no down-looking heading. Inserting up-looking data instead')
  end
  ...
  % override calculated heading offset
  if ~isnan(p.set_hdg_offset)
    hdg_offset = p.set_hdg_offset;
    disp(['>   Overriding derived heading offset with ',num2str(hdg_offset),' deg'])
  end
  
  % calculate a new pit and roll for the uplooker that is rotated
  % to the same axes as the downlooker
  [d.rol(3,:),d.pit(3,:)] = uvrot(d.rol(2,:),d.pit(2,:),-hdg_offset);
  p.up_dn_rol_off = nmean(d.rol(1,:)-d.rol(3,:));
  p.up_dn_pit_off = nmean(d.pit(1,:)-d.pit(3,:));
  ...
  if p.rotup2down~=0 & values.up==1
    ...
    elseif p.rotup2down==3
      hdg_rot_down = zeros(1,length(hrotcomp));
      hdg_rot_up = hrotcomp;
      disp('>   Rot up2down is using down compass only')
    elseif p.rotup2down==4
      hdg_rot_down = -hrotcomp;
      hdg_rot_up = zeros(1,length(hrotcomp));
      disp('>   Rot up2down is using up compass only')
    else
      hdg_rot_down = zeros(1,length(d.ru(d.izd(2),:)));
      hdg_rot_up = zeros(1,length(d.ru(d.izu(2),:)));
