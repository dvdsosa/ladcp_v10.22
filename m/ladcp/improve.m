...
% start misc_fix_compass when deviation table is given, do not
% give an error for too many large tilts, but stop gracefully
% without a result                               GK, 12.07.2013  0.7-->0.8
% catch lat,lon==NaN                             GK, 16.04.2014  0.8-->0.9
...
if (values.lat==0 & values.lon==0) | isnan(values.lat)
  ...
  values.magdev = 0;
  [a,b] = datevec(values.start_time);
  decyear = a+(values.start_time-datenum([a,1,1,0,0,0]))/365.25;
