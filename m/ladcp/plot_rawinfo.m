...
% suppress broken/weak labels when profile shallower than 200m
%                                                         GK, 08.07.2015  0.4-->0.5
...
%
if length(d.time_datenum)>200
  ind = fix(linspace(2,length(d.time_datenum)-2,200));
else
  ind = [1:length(d.time_datenum)];
end
...
if isfield(d,'tsd_m')
  ...
  if nmin(d.z_ladcp)<-200
    ...
    tay = ax(4);
    tflag = 'bottom';
    text(tax,tay*0.5,['shallow profile'],...
 	'VerticalAlignment',tflag,'HorizontalAlignment','center','fontsize',16)
    tay = ax(3);
    text(tax,tay*0.5,['info not reliable'],...
 	'VerticalAlignment',tflag,'HorizontalAlignment','center','fontsize',16)
  end
...
%=============================================
function checkbeam(t,ax,do,d_struct)
