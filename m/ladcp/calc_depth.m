...
% average pressure in case there are two ADCP pressure sensors
%                                                 GK, 04.07.2013  0.4-->0.5
% introduce linear interpolation of missing w values
%                                                 GK, 17.06.2015  0.5-->0.6
% correct mean w offset (assumes start and end at same depth)
%                                                 GK, 13.07.2015  0.6-->0.7
% replace julian/gregorian with datenum/datevec   GK, 10.02.2017  0.7-->1
...
%
if params.interpolate_missing_w==1
  good = find(isfinite(dw));
  dw = interp1(good,dw(good),[1:length(dw)]);
...
%
dt = diff(data.time_datenum)*24*3600;
...
%
  ...
  if rms(data.pres(:))>0
    ...
    % average, in case there are two sensors
    data.pres_average = nmean(data.pres,1);

    disp(['    Using LADCP pressure data'])
    zz = sw_dpth(data.pres_average,values.lat*ones(size(data.pres_average)));
    data.z = -zz;
    data.z_ladcp = -zz;
    data.z_ladcp_p = -zz;
...
i_bad = find(~isfinite(wm));
i_good = find(isfinite(wm));
wm(i_bad) = 0;
zz = cumsum(wm.*dt);


%
% do a simple fix for offset in w
%
w_offset = (zz(i_good(end))-zz(i_good(1)))/(i_good(end)-i_good(1))/dt(1);
wm = wm - w_offset;
disp(['    Correcting mean w offset of ',num2str(w_offset),' m/s'])
wm(i_bad) = 0;
zz = cumsum(wm.*dt);
