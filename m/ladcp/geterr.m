...
% replace julian/gregorian with datenum/datevec   GK, 10.02.2017  0.3-->1
...
%
% make sure that we do not interpolate beyond the values we have
%
tim = dr.tim;
uctd1 = dr.uctd;
vctd1 = dr.vctd;
if min(d.time_datenum)<tim(1) | max(d.time_datenum)>tim(end)
  disp('>   uctd timing interpolation problem')
  disp('>   using constant uctd to extrapolate')
  if min(d.time_datenum)<tim(1)
    tim = tim([1,1:end]);
    tim(1) = min(d.time_datenum);
    uctd1 = uctd1([1,1:end]);
    vctd1 = vctd1([1,1:end]);
  end
  if max(d.time_datenum)<tim(end)
    tim = tim([1:end,end]);
    tim(end) = max(d.time_datenum);
    uctd1 = uctd1([1:end,end]);
    vctd1 = vctd1([1:end,end]);
  end
end
uctd = -interp1q(tim',uctd1',d.time_datenum');
vctd = -interp1q(tim',vctd1',d.time_datenum');

[ib,it] = size(d.ru);

wm = meanmediannan(d.rw,3);
wz = gradient(-d.z,d.time_datenum*86400);
