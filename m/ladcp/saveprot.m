...
[~,ind] = nmin(abs(data.nav_time-datenum(p.time_start)));
fprintf(fid,'Start position       : %s  %s\n',num2str(data.nav_data(ind,1),'%11.5f'),...
  num2str(data.nav_data(ind,2),'%11.5f'));
[~,ind] = nmin(abs(data.nav_time-datenum(p.time_end)));
fprintf(fid,'End position         : %s  %s\n',num2str(data.nav_data(ind,1),'%11.5f'),...
  num2str(data.nav_data(ind,2),'%11.5f'));
