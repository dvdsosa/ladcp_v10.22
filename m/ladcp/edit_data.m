...
% bug in last bin masking                         GK, 14.10.2013  0.7-->0.8
% allow for second PPI                            GK, 15.07.2015  0.8-->0.9
...
if p.edit_PPI>=1
  ...
  dt = diff(d.time_datenum)*86400;

  % this is an ad-hoc fix from cruise M27/3
  % not sure whether this is always valid 
  if d.npng_d>1
    dt = dt/d.npng_d*0.85;
  end
  ...
  if p.edit_PPI>1
    ...
    PPI_hab = SS * 2*dt/2 * cos_d(d.down.Beam_angle);
    PPI_hab(find(PPI_hab > p.edit_PPI_max_hab)) = inf;
    ...
    disp(sprintf('    Previous-ping interference second echo: set %d weights to NaN',nbad));
...
if ~isempty(p.edit_skip_ensembles)
  ...
  for i=1:length(p.edit_skip_ensembles)
    if p.edit_skip_ensembles(i)
      iskip = [iskip i:length(p.edit_skip_ensembles):length(d.time_datenum)];
    end
  end
