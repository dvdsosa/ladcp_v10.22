...
% better screen output for force_adcp_ctd_lag    GK, 17.06.2015  0.4-->0.5
% catch Lat=nan                                  GK, 15.09.2015  0.5-->0.6
% replace julian/gregorian with datenum/datevec  GK, 10.02.2017  0.6-->1
...
disp(['      ',datestr(data.time_datenum(1),31),'  to  ',...
             datestr(data.time_datenum(end),31)])
if ~isempty(data.tim_sadcp)
  disp('    SADCP data coverage:')
  disp(['      ',datestr(data.tim_sadcp(1),31),'  to  ',...
               datestr(data.tim_sadcp(end),31)])
end
...
%
data.slat = data.time_datenum*nan;
data.slon = data.time_datenum*nan;
values.nav_source = 0;
if ~isempty(data.nav_data)
  disp('    Navigation data coverage:')
  disp(['      ',datestr(datevec(data.nav_time(1)),31),'  to  ',...
               datestr(datevec(data.nav_time(end)),31)])
  dummy_time = data.nav_time;
  ind = find( diff(dummy_time)>0 );
  ind = [ind;length(dummy_time)];
  lon = data.nav_data(:,2);
  lat = data.nav_data(:,1);
  data.slon = interp1(dummy_time(ind),lon(ind),data.time_datenum','linear',nan)';
  data.slat = interp1(dummy_time(ind),lat(ind),data.time_datenum','linear',nan)';
...
if all(isnan(data.slat)) 
  if ~isnan(prod(params.poss))
    ...
    if isempty( params.time_start )
      ...
      data.slat = linspace(params.poss(1)+params.poss(2)/60,...
    	params.pose(1)+params.pose(2)/60,length(data.time_datenum));
      data.slon = linspace(params.poss(3)+params.poss(4)/60,...
    	params.pose(3)+params.pose(4)/60,length(data.time_datenum));
    else
      p1 = params.poss(1)+params.poss(2)/60;
      p2 = params.pose(1)+params.pose(2)/60;
      t1 = datenum( params.time_start );
      t2 = datenum( params.time_end );
      data.slat = p1 + (p2-p1) * (data.time_datenum-t1)/(t2-t1);
      p1 = params.poss(3)+params.poss(4)/60;
      p2 = params.pose(3)+params.pose(4)/60;
      data.slon = p1 + (p2-p1) * (data.time_datenum-t1)/(t2-t1);
    end
  ...
  elseif isfield(data,'lat_sadcp')
    if ~isempty(data.lat_sadcp)
      ...
      data.slon = interp1(data.tim_sadcp(ind+1),data.lon_sadcp(ind+1),data.time_datenum);
      data.slat = interp1(data.tim_sadcp(ind+1),data.lat_sadcp(ind+1),data.time_datenum);
...
if ~isempty(data.ctdtime_data)
  ...
  disp(['      ',datestr(datevec(data.ctdtime_time(1)),31),'  to  ',...
               datestr(datevec(data.ctdtime_time(end)),31)])
  disp(' ')

  % catch out of range CTD-time data
  if min(data.ctdtime_time)>max(data.time_datenum) |...
    max(data.ctdtime_time)<min(data.time_datenum)
    ...
    dtadcp = nmedian( diff(data.time_datenum) );
...
%    wctd = wctd(ind);
    ...
    warning off
    data_int = interp1(ctdtime_time,data.ctdtime_data,data.time_datenum','nearest');
    data.wctd = interp1(ctdtime_time,wctd,data.time_datenum','nearest')';
    warning on
    ...
    dt = mean(diff(data.time_datenum));
    ...
    adcptw = [data.time_datenum', w'];

    if ~isempty(params.forced_adcp_ctd_lag)
      lag = params.forced_adcp_ctd_lag;
      lagdt = -lag*dtctd;
      co = nan;
      disp(['    Forcing W lag at: ',int2str(lag),' CTD scans ~',...
           int2str(lagdt*24*3600),' seconds']);
    else
    ...
    data_int = interp1(data.ctdtime_time-lagdt,data.ctdtime_data,...
      data.time_datenum','linear');
    data.wctd = interp1(data.ctdtime_time-lagdt,wctd,data.time_datenum','nearest')';
    warning on
    data.z = -sw_dpth(data_int(:,1),lat)';
    data.ctdtime_data = data_int;
    data.ctdtime_time = data.time_datenum;
