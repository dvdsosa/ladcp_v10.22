...
% properly named new rdi_std when no averaging is done. Do no
% averaging when profile is shallower than 100m.  
% Catch too shallow profles                       GK, 13.07.2013  0.3-->0.4
% better handle only uplooker                     GK, 13.07.2015  0.4-->0.5
% replace julian/gregorian with datenum/datevec   GK, 10.02.2017  0.5-->1
...
p.superens_std_min = data.down.Single_Ping_Err/sqrt(data.down.Pings_per_Ensemble);
if isempty(p.superens_std_min)
  p.superens_std_min = data.up.Single_Ping_Err/sqrt(data.down.Pings_per_Ensemble);
end
...
if (values.maxdepth<100 & isnan(p.avens)) | (p.avens==1 & p.avdz==0)
  disp(['    Profile is shallower than 100m. Will use fixed ensemble averaging.'])
  p.avens = 1;
  p.avdz = 0;
end
...
if (isnan(p.avdz) | p.avdz<=0 ) & (isnan(p.avens) | p.avens < 2)
  ...
  di.hbot = data.hbot;
  % make up std
  di.bvels = data.bvel'*0+values.rdi_std(1);
  di.hdg = data.hdg;
  di.pit = data.pit;
  di.rol = data.rol;
  di.temp = data.temp;
  ...
  di.dtiv = data.z*0+1;
  di.time_datenum = data.time_datenum;
  di.z = data.z;
  di.izm = data.izm;
  di.slat = data.slat;
  di.slon = data.slon;
  ...
  while ilast<il 
    ...
    % time
    di.time_datenum(im) = nmean(data.time_datenum(i1));
...
if length(ii)>0
    disp(['    Removing ',int2str(length(ii)),...
...
%
% catch empty data sets, when the profiles are too shallow
%
if length(di.time_datenum)<2
  disp('>   Data set is empty. Probably a too shallow profile.')
  return
end
