...
% simplified shear if-else                       GK, 16.05.2015  0.10-->0.11
% catch case of not enough usable data           GK, 22.05.2015  0.11-->0.12
% fix bug with preset bottom depth               GK, 27.11.2015  0.12-->0.13
% replace julian/gregorian with datenum/datevec  GK, 10.02.2017  0.13-->1
...
%
% catch case of too small amount of usable data
%
if isempty(der)
  return
end
...
%  two choices, first use original data
%  second use super ensemble data
%
if ps.shear==1
  [ds,dr,messages] = calc_shear3(data,p,ps,dr,messages);
elseif ps.shear==2
  [ds,dr,messages] = calc_shear3(di,p,ps,dr,messages);
end
